...
> `.egg` 是主应用与孵化器之间的契约：运行时按它加载，孵化器按它生成，蛋管理器按它迁移。

## 1. 目录结构

```
背单词.egg/                  ← 一个文件夹（分发时可 zip，后缀 .egg）
├── manifest.json            ← 蛋的身份证
├── index.html               ← 入口，唯一约定
├── (其余 js/css/assets 随意)
└── data/
...
| `wish` | 用户原始愿望原文。"许愿升级"时与现有代码一起喂给孵化器（决策 D5） |
| `createdBy` | 孵化留档：模型与管线版本，用于统计孵化质量 |
...
- **孵化**：孵化器按本规范生成，经 validate（schema + 静态检查）与 test（无头运行）后入蛋架。
- **升级**：仅通过主应用"许愿升级"入口（决策 D5），升级前自动整蛋备份，涉及 schema 变更时孵化器负责迁移脚本。
