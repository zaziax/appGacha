...
function buildUpgradeSystemPrompt(templateDir: string, lang: 'zh' | 'en'): string {
  ...
  const role = zh
    ? [
        ...
        '- **不要 read_file 读 vendor/ 或 base.css、icons.svg、EGGDOC.md、icon.svg**：这些是系统文件，快照已说明其内容'
      ].join('\n')
    : [
        ...
        '- **Do NOT read_file vendor/, base.css, icons.svg, EGGDOC.md, or icon.svg**: these are system files; the snapshot already describes their contents'
      ].join('\n')
