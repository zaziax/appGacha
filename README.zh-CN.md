...
- **云同步（Pro）** — 每颗蛋独立同步，互不干扰。打开蛋自动拉取最新，关闭蛋自动推送改动。单蛋可关闭同步
...
- **Bridge API v1** — 8 个需授权的能力域 + 2 个免授权 UI 操作，全部异步，`egg.d.ts` 提供完整类型声明：
  ...
  | 文件 | `fs` | `egg.fs.read()` / `write()` / `list()`（限 data/ 目录） |
  ...
  | 联机 | `network` | `egg.net.createRoom()` / `findRooms()` / `joinRoom()`（WebRTC P2P） |
  | UI（免授权） | — | `egg.ui.toast()` / `confirm()` / `pickFile()` / `saveFile()` |
  | 窗口控制（免授权） | — | `egg.minimize()` / `maximize()` / `close()` |

- **蛋模板 + 脚手架** — 智能体从模板起步，模板自带桌面应用壳设计系统（`base.css`）、Lucide 图标库（~2,000 图标）和预置 vendor ESM 库：
  - Three.js — 3D 渲染
  - Chart.js — 图表
  - marked — Markdown 解析
  - qrcode — 二维码生成
...
cd appgacha
npm install
npm start              # 完整构建（tsc + vite）→ 启动 Electron
```
...
| 通道 | 说明 |
|---|---|
| **直连** | 用户自配 OpenAI 兼容接口的 API Key（DeepSeek、Kimi、Qwen 等），key 经 Windows DPAPI 加密存储 |
| **代理** | 平台后端代理请求，扣减用户余额，设备不留 key |

两个通道通过统一的 `AiEndpoint` 抽象，系统其余部分不感知通道差异。
...
│   │   ├── aiChannel.ts         #   AI 双通道（直连 key vs 平台代理）
│   │   ├── sync.ts              #   单蛋云同步（hash→plan→upload/download/skip）
...
│   ├── icons.svg                #   Lucide SVG sprite（1000+ 图标）
...
| **M2** 能力层 + 收藏柜 | ✅ 完成 | 8 个 bridge 能力域、收藏柜基础 UI、模型配置（直连 + 代理） |
...
| **M5** 金标愿望集 | 🔜 下一步 | 约 10 条标准愿望作为回归基准，量化出蛋率 |
| **M6** 跨平台 | 📋 计划中 | macOS / Linux 移植 |
...
任何 OpenAI 兼容接口均可：DeepSeek、Kimi、Qwen、GPT-4 等。在设置中配置 base URL + 模型名 + API Key。平台也提供代理通道，余额扣费，key 不留设备。
</details>
